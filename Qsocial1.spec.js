...
test('Validacion de filtrado en redes', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Social Listening & Analítica' })).toContainText('Social Listening & Analítica');
  
  await page.getByRole('link', { name: 'Social Listening & Analítica' }).click();
  //Validacion de pagina correcta
  await expect(page.getByRole('link', { name: 'Redes Redes' })).toContainText('Redes');
  const page1Promise = page.waitForEvent('popup');
  await page.getByRole('link', { name: 'Redes Redes' }).click();
  const page1 = await page1Promise;
  // Valida que el titulo contenga lo siguiente
